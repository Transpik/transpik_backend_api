...
async function createRefundHandler(request, response) {
    ...
    try {
        const payment = await Payment.findById(payment_id, { session }).exec();
        if(!payment) throw new Error('payment not exists');

        const order = await Order.findById(payment.order_id, { session }).exec();
        ...
        const refund = await Refund.create({ order_id: order._id, payment_id: payment._id }, { session }).exec();
